...
test('Coordinator resolves a dynamically selected repository into a verified snapshot', async () => {
  ...
  try {
    ...
    await mkdir(path.join(source, 'Assets'), { recursive: true }); await mkdir(path.join(source, 'ProjectSettings'), { recursive: true });
    ...
    const resolved = await resolver.resolve({ repository, requestedRef: 'suu/test' });
    ...
    const workspace = path.join(dataDir, 'workspaces', 'job'); await snapshotStore.materializeWorkspace({ snapshotId: resolved.sourceSnapshotId, workspacePath: workspace }); assert.deepEqual(await readFile(path.join(workspace, 'Assets', 'model.bin')), object); await assert.rejects(() => readFile(path.join(workspace, '.git')), /ENOENT/);
