...
test('Coordinator resolves LFS pointers into a verified snapshot before Worker materialization', async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'resolver-'));
  let snapshotStore;
  try {
    const source = path.join(directory, 'source'); await mkdir(source);
    await git(['init', '-b', 'main'], source); await git(['config', 'user.email', 'test@example.com'], source); await git(['config', 'user.name', 'Test'], source);
    const object = Buffer.from('large materialized object');
    const oid = createHash('sha256').update(object).digest('hex');
    await mkdir(path.join(source, 'Assets'), { recursive: true });
    await mkdir(path.join(source, 'ProjectSettings'), { recursive: true });
    await writeFile(path.join(source, '.gitattributes'), 'Assets/*.bin filter=lfs diff=lfs merge=lfs -text\n');
    await writeFile(path.join(source, 'Assets', 'model.bin'), `version https://git-lfs.github.com/spec/v1\noid sha256:${oid}\nsize ${object.length}\n`);
    await writeFile(path.join(source, 'ProjectSettings', 'ProjectVersion.txt'), 'm_EditorVersion: 6000.0.59f2\n');
    ...
    const fakeClient = {
      ...
      async downloadPlannedObject({ destinationPath }) { await writeFile(destinationPath, object, { flag: 'wx' }); },
    };
    const config = {
      repository: {
        alias: 'project', sshUrl: source,
        sourceDependencies: { gitLfs: { enabled: true, endpointUrl: 'https://github.com/example/project.git/info/lfs' }, submodules: { enabled: false } },
      },
    ...
    const resolved = await resolver.resolve({ requestedRef: 'suu/test' });
    assert.equal(resolved.unityVersion, '6000.0.59f2');
    assert.equal(resolved.sourceSnapshotManifest.lfs.objectCount, 1);
    const workspace = path.join(dataDir, 'workspaces', 'job');
    await snapshotStore.materializeWorkspace({ snapshotId: resolved.sourceSnapshotId, workspacePath: workspace });
    assert.deepEqual(await readFile(path.join(workspace, 'Assets', 'model.bin')), object);
    await assert.rejects(() => readFile(path.join(workspace, '.git')), /ENOENT/);
