...
import { CiError } from '../src/core/errors.js';
...
test('Worker consumes only the published snapshot and performs no Git/LFS operation', async () => {
  ...
  try {
    ...
    const dependencyRestorer = {
      async restore({ projectPath }) {
        assert.equal(projectPath, path.join(directory, 'workspace', 'UnityProject'));
    ...
    const unityService = {
      ...
      async build({ onSpawn, projectPath }) { assert.equal(projectPath, path.join(directory, 'workspace', 'UnityProject')); executionOrder.push('build'); onSpawn(); return { path: apkPath, name: 'app.apk', logPath: path.join(directory, 'unity.log') }; },
    };
    ...
    worker = new BuildWorker({ config: { dataDir: directory, runner: { pollIntervalMs: 10, heartbeatIntervalMs: 20 } }, store, adapters, statusService, snapshotStore, unityService, dependencyRestorer, artifactVerifier, artifactPublisher, logger, redactor: { redact(value) { return value; } } });
    worker.start(); worker.wake();
    await waitUntil(() => store.getJob(job.id).status === 'SUCCEEDED');
    assert.equal(materialized, 1);
    assert.deepEqual(executionOrder, ['restore', 'build']);
...
test('Worker aborts dependency restore on shutdown and leaves the snapshot job recoverable', async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'worker-restore-abort-'));
  const store = new JobStore(path.join(directory, 'jobs.sqlite3'));
  const adapter = new FakeAdapter(); const adapters = new AdapterRegistry([adapter]); const statusService = new StatusService({ store, adapters, logger });
  let worker;
  try {
    const { job } = store.createJob({ platform: 'slack', workspaceId: 'T1', channelId: 'C1', sourceMessageId: '200.1', requesterId: 'U1', repositoryAlias: 'project', projectPath: 'UnityProject', requestedBranch: 'develop', buildProfilePath: 'Assets/BuildProfiles/PICO.asset' });
    store.setThread(job.id, '200.1');
    const manifest = { snapshotId: 'e'.repeat(64), repositoryId: 'project', commitSha: 'f'.repeat(40), filesDigest: 'a'.repeat(64), lfs: { enabled: true, objectCount: 0, totalSizeBytes: 0, objects: [] }, createdAt: new Date().toISOString() };
    store.setResolvedSource(job.id, { commitSha: manifest.commitSha, sourceSnapshotId: manifest.snapshotId, sourceSnapshotManifest: manifest, unityVersion: '6000.0.59f2' });
    store.setQueued(job.id);
    let cleaned = false;
    ...
    let buildCalls = 0;
    const snapshotStore = {
      async materializeWorkspace({ workspacePath }) { await mkdir(workspacePath, { recursive: true }); },
      async cleanupWorkspace(workspacePath) { cleaned = true; await rm(workspacePath, { recursive: true, force: true }); },
    };
    const dependencyRestorer = {
      async restore({ signal }) {
        restoreStarted = true;
        await new Promise((resolve, reject) => {
          const abort = () => reject(new CiError({ code: 'NUGET_RESTORE_ABORTED', category: 'RUNNER_ERROR', message: 'aborted', stage: 5 }));
    ...
    const unityService = {
      async inspectProject() { return { unityVersion: '6000.0.59f2', unityExecutable: '/fake/Unity', projectPath: path.join(directory, 'workspace', 'UnityProject') }; },
      async build() { buildCalls += 1; throw new Error('build must not start'); },
    };
    worker = new BuildWorker({
      config: { dataDir: directory, runner: { pollIntervalMs: 10, heartbeatIntervalMs: 20 } },
      store,
      adapters,
      statusService,
      snapshotStore,
      ...
      artifactPublisher: {},
      logger,
      redactor: { redact(value) { return value; } },
    });
    worker.start(); worker.wake();
    ...
    assert.equal(store.listEvents(job.id).some((event) => event.eventType === 'RUNNER_SHUTDOWN_INTERRUPTED_JOB' && event.stage === 5), true);
